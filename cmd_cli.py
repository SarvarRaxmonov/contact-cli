...
from cli_print import print_all_contact,logo, entering_menu, menu,entering_menu,print_searched_contacts
os.system("cls")
colorama.init()
db = Server()
    ...
    * exit (Yani offline datasini parchalab o'zi chiqib ketadi)
    ...
    def update_contact(self): 
        ...
        if click_id != 0:
            ...
            if check == True:
                db.updating_contacts_to_user_profile(self.USER,contact_id=click_id,new_name=click_new_name,new_phone_number=click_new_phone)
    ...
    def delete_contact(self):
        cn = print_all_contact(user=self.USER)
        click_id = int(input(Fore.LIGHTRED_EX + "\n  Delete qilmoqchi bulgan contact ID yozing : ") or 0)
        if click_id != 0:
            check = db.is_user_contact_id_exists(user=self.USER,contc_id=click_id)
            if check == True:
